...
#---------------------------------------------------------------------------------------
                time0 = time.time()
                current_push_policy = local_policy_push.tensor_[idics][:, iter_num]
                ...
                for i in range(trainers_per_server):
                    ...
                    local_idx_split_size.append(th.tensor([idx_i.shape[0]], dtype=th.int64, device="cpu"))
                ...
                idx_gather_list = [
                    th.empty((int(num_emb),), dtype=idics.dtype, device="cpu")
                    for num_emb in gather_list
                ]
                local_utils.alltoallv_cpu(
                    th.distributed.get_rank(local_group),
                    th.distributed.get_world_size(local_group),
                    idx_gather_list,
                    local_idx_list,
                ...
                time3 = time.time()
                idx_to_buffer = th.cat(idx_gather_list, dim=0).to(device)
                grad_to_buffer = th.cat(grad_gather_list, dim=0).to(device)

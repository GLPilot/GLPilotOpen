...
def run(args, device, data):
    ...
    #     writer = SummaryWriter(f'runs/dgl_fanout5,10,15_breakdown_4part_{timestamp}/part_{g.rank()}')
    ...
    for epoch in range(args.num_epochs):
        ...
        update_time = 0
        update_orig_time = 0 
        update_optimizer_time = 0
        
        update_orig_part1_time = 0
        update_orig_part2_time = 0
        update_orig_part3_time = 0
        ...
        with model.join():
            ...
            for step, (input_nodes, seeds, blocks) in enumerate(dataloader):
                ...
                load_end = time.time()
                load_time1 += load_end - load_start
                # Compute loss and prediction
                load_start = time.time()
                batch_inputs = emb_layer(input_nodes)
                load_end = time.time()
                load_time2 += load_end - load_start
                start = time.time()
                ...
                # ====== update begin ======
                emb_optimizer.step()
                update_orig_step_time, update_orig_step_part1_time, update_orig_step_part2_time, update_orig_step_part3_time = 0, 0, 0, 0
                update_optimizer_begin = time.time()
                optimizer.step()
                update_optimizer_end = time.time()
                update_time += time.time() - compute_end
                update_orig_time += update_orig_step_time
                update_optimizer_time += update_optimizer_end - update_optimizer_begin
                update_orig_part1_time += update_orig_step_part1_time
                update_orig_part2_time += update_orig_step_part2_time
                update_orig_part3_time += update_orig_step_part3_time
                # ====== update end ======
                ...
                global_step += 1 # global step for tensorboard record
                if global_step % args.log_every == 0: #TCJ use global step count
                    ...
                    #tensorboard writer 
                    if args.tensorboard:
                        writer.add_scalar(f'train/epoch', epoch, global_step)
                        writer.add_scalar(f'train/Loss', loss.item(), global_step)
                        writer.add_scalar(f'train/Accuracy', acc.item(), global_step)
                        writer.add_scalar(f'train/Speed (samples/sec)', np.mean(iter_tput[3:]), global_step)
                        writer.add_scalar(f'train/GPU Memory (MB)', gpu_mem_alloc, global_step)
                        writer.add_scalar(f'train/step_time_s', np.sum(step_time[-args.log_every:]), global_step)
        ...
        print("epoch:{},"
            "Part {}, Epoch Time(s): {:.4f}, sample: {:.4f}, data_copy: {:.4f}, loademb:  {:.4f}, forward"
        ...
        if args.tensorboard:
            writer.add_scalar(f'perf_data/Epoch Time (s)', toc - tic, epoch)
            writer.add_scalar(f'perf_data/Sample+Data Copy Time (s)', sample_time, epoch)
            writer.add_scalar(f'perf_data/load_time Time (s)', load_time, epoch)
            writer.add_scalar(f'perf_data/Forward Time (s)', forward_time, epoch)
            writer.add_scalar(f'perf_data/Backward Time (s)', backward_time, epoch)
            
            writer.add_scalar(f'perf_data/Update Time (s)', update_time, epoch)
            writer.add_scalar(f'perf_data/Update Orig Time (s)', update_orig_time, epoch)
            writer.add_scalar(f'perf_data/Update Orig Part1 Time (s)', update_orig_part1_time, epoch)
            writer.add_scalar(f'perf_data/Update Orig Part2 Time (s)', update_orig_part2_time, epoch)
            writer.add_scalar(f'perf_data/Update Orig Part3 Time (s)', update_orig_part3_time, epoch)
            writer.add_scalar(f'perf_data/Update Optimizer Time (s)', update_optimizer_time, epoch)

            writer.add_scalar(f'perf_data/Number of Seeds', num_seeds, epoch)
            writer.add_scalar(f'perf_data/Number of Inputs', num_inputs, epoch)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tensorboard", type=bool, default=False, help="if use tensorboard to log"
    )

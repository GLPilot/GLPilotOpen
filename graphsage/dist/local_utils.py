...
def alltoall_cpu(rank, world_size, output_tensor_list, input_tensor_list, group=None):
    ...
    """
    zero_rank_base = (dist.get_rank() // world_size) * world_size
    input_tensor_list = [
        tensor.to(th.device("cpu")) for tensor in input_tensor_list
    ]
    for i in range(world_size):
        dist.scatter(
            output_tensor_list[i], input_tensor_list if i == rank else [], src=zero_rank_base + i, group=group
        )


def alltoallv_cpu(rank, world_size, output_tensor_list, input_tensor_list, group=None):
    """Each process scatters list of input tensors to all processes in a cluster
    ...
    for i in range(world_size):
        if i == rank:
            ...
            sender = dist.isend(
                input_tensor_list[i].to(th.device("cpu")), dst=zero_rank_base + i, group=group
            )
            senders.append(sender)

    for i in range(world_size):
        if i != rank:
            dist.recv(output_tensor_list[i], src=zero_rank_base + i, group=group)

    th.distributed.barrier(group=group)
...
def alltoall_nccl(rank, world_size, output_tensor_list, input_tensor_list, device, group=None):
    """Each process scatters list of input tensors to all processes in a cluster
    and return gathered list of tensors in output list. The tensors should have the same shape.
    ...
    device: th.device
        Device of the tensors
    """
    assert th.distributed.get_backend() == "nccl"
    if th.distributed.get_backend() == "nccl":
        input_tensor_list = [
            tensor.to(th.device(device)) for tensor in input_tensor_list
        ]
        th.distributed.all_to_all(output_tensor_list, input_tensor_list, group=group)
    else:
        alltoall_cpu(
            rank,
            world_size,

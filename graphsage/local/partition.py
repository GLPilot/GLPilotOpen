...
from dgl import sort_csc_by_tag, sort_csr_by_tag
...
def partition_graph(
    ...
    * ``balance_edges`` balances the number of edges in each partition.
    ...
    print(f"list(g.formats().values()):{list(g.formats().values())}")
    ...
    #TCJ 预定义combine graph
    start = time.time()
    print(
        "Create combined graph {:.3f} seconds, peak memory: {:.3f} GB".format(
            time.time() - start, get_peak_mem()
        )
    )
    start = time.time()
    for part_id in range(num_parts):
        print(f"------------------part {part_id} Begin--------------------------")
        part = parts[part_id]
        ...
        if num_parts > 1:
            ...
            for ntype in g.ntypes:
                ...
                # print(f"inner_node_mask:{dir(inner_node_mask)}")
                print(f"inner_node_mask.shape:{inner_node_mask.shape}")
                print(f"inner_node_mask.sum().item():{inner_node_mask.sum().item()}")

                # This is global node IDs.
                local_nodes = F.boolean_mask(
                    part.ndata[ndata_name], inner_node_mask
                )
                print(f"local_nodes.shape:{local_nodes.shape}")
                print(f"local_nodes:{local_nodes}")

                print(f"part.ndata:{part.ndata}")
                # part.remove_nodes(2206884)#TCJ cant rm node out of range
                print(f"part.ndata['_ID']:{part.ndata['_ID']}")
                print(f"part.ndata['inner_node']:{part.ndata['inner_node']}")
                print(f"part.ndata['part_id']:{part.ndata['part_id']}")
                print(f"(part.ndata['part_id'] != 0).sum().item():{(part.ndata['part_id'] != 0).sum().item()}")
                print(f"part.ndata['part_id'].nonzero(as_tuple=True)[0][0].item():{part.ndata['part_id'].nonzero(as_tuple=True)[0][0].item()}")

                print(f"part.ndata['orig_id']:{part.ndata['orig_id']}")

                print(f"part.ndata['_ID']:{part.ndata['_ID'].shape}")
                print(f"part.ndata['inner_node']:{part.ndata['inner_node'].shape}")
                print(f"part.ndata['part_id']:{part.ndata['part_id'].shape}")
                print(f"part.ndata['orig_id']:{part.ndata['orig_id'].shape}")
                print(f"dir(part.ndata):{dir(part.ndata)}")
            ...
            for etype in g.canonical_etypes:
                ...
                print(f"inner_edge_mask.shape:{inner_edge_mask.shape}")
                # This is global edge IDs.
                local_edges = F.boolean_mask(
                    part.edata[edata_name], inner_edge_mask
                )
                print(f"local_edges.shape:{local_edges.shape}")

                print(f"part.edata:{part.edata}")
                print(f"part.edata['_ID']:{part.edata['_ID']}")
                print(f"part.edata['orig_id']:{part.edata['orig_id']}")
                print(f"part.edata['inner_edge']:{part.edata['inner_edge']}")
                print(f"torch.count_nonzero(part.edata['inner_edge')]:{torch.count_nonzero(part.edata['inner_edge'])}")
                print(f"(part.edata['inner_edge']== 0).nonzero(as_tuple=True)[0]:{(part.edata['inner_edge']== 0).nonzero(as_tuple=True)[0]}")

                print(f"part.edata['_ID'].shape:{part.edata['_ID'].shape}")
                print(f"part.edata['orig_id'].shape:{part.edata['orig_id'].shape}")
                print(f"part.edata['inner_edge'].shape:{part.edata['inner_edge'].shape}")
                
                print(f"dir(part.edata):{dir(part.edata)}")
        ...
        print(f"node_feat_file:{node_feat_file}")
        print(f"node_feats:{node_feats}")
        print(f"node_feats.items():{node_feats.items()}")
        print(f"node_feats['_N/features'].shape:{node_feats['_N/features'].shape}")
        print(f"node_feats['_N/labels'].shape:{node_feats['_N/labels'].shape}")
        print(f"node_feats['_N/train_mask'].shape:{node_feats['_N/train_mask'].shape}")
        print(f"node_feats['_N/val_mask'].shape:{node_feats['_N/val_mask'].shape}")
        print(f"node_feats['_N/test_mask'].shape:{node_feats['_N/test_mask'].shape}")

        print(f"node_feats['_N/features'].dtype:{node_feats['_N/features'].dtype}")
        print(f"node_feats['_N/labels'].dtype:{node_feats['_N/labels'].dtype}")
        print(f"node_feats['_N/train_mask'].dtype:{node_feats['_N/train_mask'].dtype}")
        print(f"node_feats['_N/val_mask'].dtype:{node_feats['_N/val_mask'].dtype}")
        print(f"node_feats['_N/test_mask'].dtype:{node_feats['_N/test_mask'].dtype}")
        
        print(f"edge_feat_file:{edge_feat_file}")
        print(f"edge_feats:{edge_feats}")
        sort_etypes = len(g.etypes) > 1
        print("------------------------")
        print(f"part_graph_file:{part_graph_file}")
        print(f"type(part):{type(part)}")
        print(f"dir(part):{dir(part)}")
        print(f"part:{part}")
        print(f"graph_formats:{graph_formats}")
        print(f"sort_etypes:{sort_etypes}")
        print(f"part.edges:{part.edges}")
        # print(f"part.adj_external(scipy_fmt='coo'):{part.adj_external(scipy_fmt='coo')}")
        adjacency_matrix = part.adjacency_matrix()
        print(f"adjacency_matrix:{adjacency_matrix}")
        print(f"adjacency_matrix.indices[0].shape:{adjacency_matrix.indices().shape}")
        print(f"adjacency_matrix.indices[0].dtype:{adjacency_matrix.indices().dtype}")
        print(f"adjacency_matrix.values.shape:{adjacency_matrix.val.shape}")
        print(f"adjacency_matrix.values.dtype:{adjacency_matrix.val.dtype}")
    ...
    print(f"orig_nids:{orig_nids}")
    print(f"orig_nids.shape:{orig_nids.shape}")
    print(f"orig_eids:{orig_eids}")
    print(f"orig_eids.shape:{orig_eids.shape}")
    print(f"g:{g}")
    print(f"g.ndata['features']:{g.ndata['features']}")
    print(f"g.ndata['labels']:{g.ndata['labels']}")
    print(f"g.ndata['train_mask']:{g.ndata['train_mask']}")
    print(f"g.ndata['val_mask']:{g.ndata['val_mask']}")
    print(f"g.ndata['test_mask']:{g.ndata['test_mask']}")
    
    print(f"g.ndata['features'].shape:{g.ndata['features'].shape}")
    print(f"g.ndata['labels'].shape:{g.ndata['labels'].shape}")
    print(f"g.ndata['train_mask'].shape:{g.ndata['train_mask'].shape}")
    print(f"g.ndata['val_mask'].shape:{g.ndata['val_mask'].shape}")
    print(f"g.ndata['test_mask'].shape:{g.ndata['test_mask'].shape}")
    
    for index, feature in enumerate(g.ndata['features'][:10, :5]):
        print(f"Index {index}: {feature}")
    g.ndata['features'] = g.ndata['features'][orig_nids]
    for index, feature in enumerate(g.ndata['features'][:10, :5]):
        print(f"Index {index}: {feature}")
    g.ndata['labels'] = g.ndata['labels'][orig_nids]
    g.ndata['train_mask'] = g.ndata['train_mask'][orig_nids]
    g.ndata['val_mask'] = g.ndata['val_mask'][orig_nids]
    g.ndata['test_mask'] = g.ndata['test_mask'][orig_nids]

    src, dst = g.edges()
    print(f"src:{src}")
    print(f"dst:{dst}")
    nids = torch.arange(orig_nids.size(0))
    map_nid = torch.zeros_like(orig_nids)
    map_nid[orig_nids] = nids
    print(f"map_nid{map_nid[:10]}")
    new_src = map_nid[src]
    new_dst = map_nid[dst]
    ...
    new_g.ndata['features'] = g.ndata['features']
    ...
    for index, feature in enumerate(new_g.ndata['features'][:10, :5]):
        print(f"feature {index}: {feature}")

    dgl.save_graphs(out_path + "/combined_graph.dgl", new_g)
    ...
    # print(f"g.nodes().shape:{g.nodes().shape}")
    # print(f"g.edges()[0]:{g.edges()[0]}")

    # for index, (src, dst) in enumerate(zip(g.edges()[0][:10], g.edges()[1][:10])):
    #     print(f"edge {index}: {src},{dst}")
    


    # src, dst = g.edges()
    # sorted_src = src[orig_eids]
    # sorted_dst = dst[orig_eids]
    # g_sorted = dgl.graph((sorted_src, sorted_dst))
    # g_sorted.ndata['features'] = g.ndata['features']
    # g_sorted.ndata['labels'] = g.ndata['labels']
    # g_sorted.ndata['train_mask'] = g.ndata['train_mask']
    # g_sorted.ndata['val_mask'] = g.ndata['val_mask']
    # g_sorted.ndata['test_mask'] = g.ndata['test_mask']

    # for index, (src, dst) in enumerate(zip(g_sorted.edges()[0][:10], g_sorted.edges()[1][:10])):
    #     print(f"edge {index}: {src},{dst}")
    # for index, feature in enumerate(g_sorted.ndata['features'][:10, :5]):
    #     print(f"feature {index}: {feature}")

    # g.edges()[0] = g.edges()[0][orig_eids]
    # g.edges()[1] = g.edges()[1][orig_eids]
    # for index, src in enumerate(g.edges()[0][:10]):
    #     print(f"src {index}: {src}")
    # for index, dst in enumerate(g.edges()[1][:10]):
    #     print(f"dst {index}: {dst}")

...
class DistEmbedding:
    ...
    def __call__(self, idx, device=torch.device("cpu"), step=None):
        ...
        # Prepare tensor to store the output embeddings
        emb = torch.zeros((len(idx), self._embedding_dim), device=device)

        lifetime_greater_than_one = self.lifetime_tracker[idx] >= 1
        if lifetime_greater_than_one.any():
            emb[lifetime_greater_than_one] = self.embedding_buffer[idx[lifetime_greater_than_one]].to(device)
        if (~lifetime_greater_than_one).any():
            from_tensor = idx[~lifetime_greater_than_one]
            emb[~lifetime_greater_than_one] = self._tensor.tensor_[from_tensor].to(device, non_blocking=True)
            new_lifetime = self.generate_lifetime(from_tensor)
            lifetime_to_store = new_lifetime > 1
            if lifetime_to_store.any():
                self.embedding_buffer[from_tensor[lifetime_to_store]] = emb[~lifetime_greater_than_one][lifetime_to_store].cpu()
                self.lifetime_tracker[from_tensor[lifetime_to_store]] = new_lifetime[lifetime_to_store]
...
def train(
    ...
    # check sampling data
    emb_layer.sparse_emb.get_total_access_tensor()
    dist.barrier()
    emb_layer.sparse_emb.get_access_tensor_sum()
    dist.barrier()
    # emb_layer.sparse_emb.print_local_access_tensor_list()
    # for i in range(num_pseduo_iters):
    #     emb_layer.sparse_emb.sliding_win_avg(i+1)

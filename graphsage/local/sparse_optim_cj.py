...
class DistSparseGradOptimizer(abc.ABC):
    ...
    def __init__(self, params, lr, part_config=None):
        self._params = params
        self._lr = lr
        self._rank = None
    ...
    # def remove_common_indices(self, input_nodes, grad, indices):
    #     # 将 indices 转换为集合，以便快速查找
    #     # indices_set = set(indices.tolist())
        
    #     # # 创建布尔掩码，表示 input_nodes 中的哪些元素不在 indices 中
    #     # mask = torch.tensor([node.item() not in indices_set for node in input_nodes])
    ...
    #         mask = torch.isin(input_nodes, indices)
    #         remaining_nodes = input_nodes[~mask]
    #         remaining_grad = grad[~mask]
    #     else:
    #         remaining_nodes = input_nodes
    #         remaining_grad = grad
        
    #     # return remaining_nodes
    #     return remaining_nodes, remaining_grad
    ...
    def step(self):
        ...
        with th.no_grad():
            ...
            for emb in self._params:
                ...
                kvstore = None
                trainers_per_server = 1 # we only have one GPU to simulate 8 GPUs
                ...
                if self._world_size > 1:
                    ...
                    idx_split = torch.zeros_like(idics)
                    for i in range(len(self._part_config) - 1):
                        idx_split[(idics >= self._part_config[i]) & (idics < self._part_config[i+1])] = i
                    idx_split[idics >= self._part_config[-1]] = len(self._part_config) - 1
                    idx_split_size = []
                    idics_list = []
                    grad_list = []
            ...
            for emb in self._params:
                ...
                #TCJ use cv part mask
                # assert self._cv_mask is not None
                # assert self._cv_part is not None
                # idx = idx.to("cpu")
                # grad = grad.to("cpu")

                # idx_cv = self._cv_mask[idx]
                # idx = idx[idx_cv != self._cv_part]
                # grad = grad[idx_cv != self._cv_part]
...
class SparseAdam(DistSparseGradOptimizer):
    ...
    def __init__(self, params, lr, betas=(0.9, 0.999), eps=1e-08, part_config=None):
        super(SparseAdam, self).__init__(params, lr, part_config=part_config)
        self._eps = eps
        # We need to register a state sum for each embedding in the kvstore.
        self._beta1 = betas[0]
        self._beta2 = betas[1]
...
#-----------------------------------------------------------------------------
            ...
            state_step = ShmTensor(emb.name + "_step",
                                   (emb.num_embeddings,),
                                   dist.get_rank(),
                                   dist.get_world_size(),
                                   None,
            ...
            state_mem = ShmTensor(emb.name + "_mem",
                                  (emb.num_embeddings, emb.embedding_dim),
                                  dist.get_rank(),
                                  dist.get_world_size(),
                                  None,
            ...
            state_power = ShmTensor(emb.name + "_power",
                                    (emb.num_embeddings, emb.embedding_dim),
                                    dist.get_rank(),
                                    dist.get_world_size(),
                                    None,
    ...
    def update(self, idx, grad, emb):
        ...
        eps = self._eps
        clr = self._lr
        state_step, state_mem, state_power = self._state[emb.name][0].tensor_, self._state[emb.name][1].tensor_, self._state[emb.name][2].tensor_
        ...
        if state_block:
            ...
            # can use it
            std_event.wait()
        emb._tensor.tensor_[state_idx] -= std_values_dst

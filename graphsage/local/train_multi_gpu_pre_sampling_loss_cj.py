...
def train(
    proc_id, nprocs, device, g, num_classes, train_idx, test_idx, val_idx, model, use_uva, emb_layer, emb_optimizer, args
):
    ...
    # scheduler = lr_scheduler.CosineAnnealingLR(opt, T_max=20, eta_min=args.lr_min) #TCJ viariable lr 
    for epoch in range(args.num_epoch):
        ...
        if dist.get_rank() == 0:
            ...
            #cold,hot_mask
            # cold_mask = summed_sample_count < np.percentile(summed_sample_count_rm_zero, 50)
            # hot_mask = ~cold_mask
            # mask = cold_mask
            ...
            # print(f"hot_mask.shape:{hot_mask.shape}")
            # node_cv_hotness_masked = node_cv[mask]
            
            #get cold cv mask
            node_cv = torch.from_numpy(node_cv)
            cv_mask = torch.zeros_like(node_cv, dtype=torch.int)
            cv_mask[(node_cv > 1.7)] = 0
            cv_mask[(node_cv > 1.3) & (node_cv <= 1.7)] = 1
            cv_mask[(node_cv > 0.9) & (node_cv <= 1.3)] = 2
            cv_mask[(node_cv > 0.5) & (node_cv <= 0.9)] = 3
            cv_mask[(node_cv > 0.0) & (node_cv <= 0.5)] = 4
            cv_mask[(node_cv < 0)] = 5


        if dist.get_rank() == 0:
            cv_mask = cv_mask.to(device)
        else: 
            cv_mask = torch.zeros(g.num_nodes(), dtype=torch.int, device=device)
        dist.broadcast(cv_mask, src=0)

        emb_optimizer._cv_part = args.frozen_cv_part
        emb_optimizer._cv_mask = cv_mask.to("cpu")

            
        pre_sample_end = time.time()
        if dist.get_rank() == 0:
            print(f"pre-sampling epoch: {epoch} finish, Elapsed time: {pre_sample_end - pre_sample_start}" )
        ...
        for it, (input_nodes, output_nodes, blocks) in enumerate(
            ...
            # output_nodes = remove_common_indices(output_nodes, indices)
            ...
            fetch_embedding_sum = fetch_embedding_sum + (fetch_embedding_end - fetch_embedding_start)
            
            forward_start = time.time()
            y_hat = model(blocks, x)
            forward_end = time.time()
            forward_sum = forward_sum + (forward_end - forward_start)
            ...
            backward_sum = backward_sum + (backward_end - backward_start)

            # emb_layer.sparse_emb.process_gradients()
            if it % args.fake_gpus == 0 and it !=0:
                embedding_updata_start = time.time()
                emb_optimizer.step()
                opt.step()
                dist.barrier()
                embedding_updata_stop = time.time()
                embedding_updata_sum = embedding_updata_sum + (embedding_updata_stop - embedding_updata_start)
                total_loss += loss
                acc = (
        ...
        updata_sum = updata_sum + (updata_end - updata_start)
        ...
        acc_end = time.time()
        acc_sum = acc_sum + (acc_end - acc_start)

        if proc_id == 0:
            print(f"pre sampleing:{pre_sample_end-pre_sample_start}\t fetch embedding:{fetch_embedding_sum}\t forward{forward_sum}\t backward{backward_sum}\t embedding updating{embedding_updata_sum} updating{updata_sum}\t acc_sum{acc_sum}")
            print(
                "Epoch {:05d} | Loss {:.4f} | Accuracy {:.4f}".format(
                    epoch, total_loss / (it + 1), acc.item()
                )
...
def run(proc_id, nprocs, devices, g, data, args):
    ...
    train(
        ...
        num_classes,
        train_idx,
        test_idx, #TCJ mark tested train
        val_idx,
        model,
        use_uva,
        emb_layer,

...
from dgl.distributed.optim.pytorch.utils import alltoall, alltoallv
...
class DistSparseGradOptimizer(abc.ABC):
    ...
    def __init__(self, params, lr, 
        ...
        self._defaults = {}
        self.time_list = [0,0,0,0,0,0,0]

        ## for delay update
        # self.gradient_table = {
        #     emb.weight.name: th.zeros(emb.weight.shape, dtype=emb.weight.dtype)
        #     for emb in params
        # }

        # for scheduling
        # assert local_gradient_buffer is not None
        self.local_gradient_buffer = local_gradient_buffer
    ...
    def print_time(self):
        ...
        print(self._rank, " emb_optim:", self.time_list)
        self.time_list = [0,0,0,0,0,0,0]
        ...
        with th.no_grad():
            ...
            for emb in self._params:
                ...
                if self._world_size > 1:
                    ...
                    idx_split = kvstore.get_partid(emb.data_name, idics.to("cpu"))
                    idx_split_size = []
                    idics_list = []
                    grad_list = []
    ...
    def step_delay(self, mask_hot, step, k):
        """The step function.
        ...
        with th.no_grad():
            ...
            for emb in self._params:
                ...
                # 将 mask 对应的 embedding gradient 累加到额外的 gradient table 中
                if step % k != 0:
                    high_freq_mask = mask_hot[idx]
                    # 使用索引和梯度处理重复值
                    unique_idx, inverse_idx = idx[high_freq_mask].unique(return_inverse=True)
                    grad_accum = th.zeros_like(self.gradient_table[name][unique_idx])
                    grad_accum.index_add_(0, inverse_idx, grad[high_freq_mask])
                    self.gradient_table[name][unique_idx] += grad_accum
                    # 过滤掉高频采样节点的梯度
                    grad = grad[~high_freq_mask]
                    idx = idx[~high_freq_mask]
            ...
            # 在 epoch 能整除 k 时，更新 gradient table 中所有的值
            if step % k == 0:
                for emb in self._params:
                    name = emb.weight.name
                    extra_grad = self.gradient_table[name]
                    non_zero_idx = extra_grad.abs().sum(dim=1) != 0
                    self.update(
                        non_zero_idx.nonzero().squeeze().to(device, non_blocking=True),
                        extra_grad[non_zero_idx].to(device, non_blocking=True),
                        emb,
                    )
                    # 清空 gradient table
                    self.gradient_table[name].zero_()

        if self._world_size > 1:
            th.distributed.barrier()

    def step_sche(self, iter_num=None, local_policy_push=None, num_local_gpus=None, local_group=None, emb_buffer=None):
        """The step function.
        ...
        assert local_policy_push is not None

        with th.no_grad():
            device = (
                th.device(f"cuda:{th.distributed.get_rank(local_group)}")
                if th.distributed.get_backend() == "nccl"
                else th.device("cpu")
            )
            ...
            for emb in self._params:
                ...
                idics = (
                    th.cat(idics, dim=0)
                    if len(idics) != 0
                    else th.zeros((0,), dtype=th.long, device=device)
                )
                grads = (
                    th.cat(grads, dim=0)
                    if len(grads) != 0
                ...
                # device = grads.device
#---------------------------------------------------------------------------------------
                # update local part first
                if False:
                    local_mask = (idics >= self.local_start) & (idics < self.local_end)
                    local_part_index = idics[local_mask]
                    local_part_gradient = grads[local_mask]
                    remote_idics = idics[~local_mask]
                    remote_grads = grads[~local_mask]
#---------------------------------------------------------------------------------------
                    local_idx_split_size = []
                    local_idx_list = []
                    local_grad_list = []
                    local_idx_split = th.remainder(local_part_index, trainers_per_server)
                    for i in range(trainers_per_server):
                        mask = local_idx_split == i
                        idx_i = local_part_index[mask]
                        grad_i = local_part_gradient[mask]
                        local_idx_split_size.append(th.tensor([idx_i.shape[0]], dtype=th.int64, device=device))
                        local_idx_list.append(idx_i)
                        local_grad_list.append(grad_i)
                    time2 = time.time()
                    gather_list = list(th.empty([th.distributed.get_world_size(local_group)], 
                                                dtype=th.int64, device=device).chunk(th.distributed.get_world_size(local_group)))
                    local_utils.alltoall_nccl(
                        th.distributed.get_rank(local_group),
                        th.distributed.get_world_size(local_group),
                        gather_list,
                        local_idx_split_size,
                        device,
                        group=local_group
                    ...
                    grad_gather_list = [
                        th.empty((int(num_emb), grads.shape[1]), dtype=grads.dtype, device=device,)
                        for num_emb in gather_list
                    ]
                    local_utils.alltoall_nccl(
                        th.distributed.get_rank(local_group),
                    ...
                    time3 = time.time()
                    idx_to_buffer = th.cat(idx_gather_list, dim=0)
                    grad_to_buffer = th.cat(grad_gather_list, dim=0)
                    local_grad_indices, inverse, cnt = th.unique(
                        idx_to_buffer, return_inverse=True, return_counts=True
                    )
                    local_grad_values = th.zeros(
                        (local_grad_indices.shape[0], grad_to_buffer.shape[1]), device=device
                    )
                    local_grad_values.index_add_(0, inverse, grad_to_buffer)
                    idics = remote_idics
                    grads = remote_grads
#---------------------------------------------------------------------------------------
                # process rest
                time0 = time.time()
                current_push_policy = local_policy_push.tensor_[idics.to("cpu")][:, iter_num]            
                push_sync_mask = current_push_policy == 0
                push_buff_mask = current_push_policy == 1
                push_sync_mask = push_sync_mask.to(device)
                push_buff_mask = push_buff_mask.to(device)
                push_buff_idics = idics[push_buff_mask]
                push_buff_grads = grads[push_buff_mask]
                time1 = time.time()
                local_idx_split_size = []
                local_idx_list = []
                local_grad_list = []
                local_idx_split = th.remainder(push_buff_idics, trainers_per_server)
                for i in range(trainers_per_server):
                    mask = local_idx_split == i
                    idx_i = push_buff_idics[mask]
                    grad_i = push_buff_grads[mask]
                    local_idx_split_size.append(th.tensor([idx_i.shape[0]], dtype=th.int64, device=device))
                    local_idx_list.append(idx_i)
                    local_grad_list.append(grad_i)
                time2 = time.time()
                ...
                grad_values = th.zeros(
                    (grad_indices.shape[0], grad_to_buffer.shape[1]), device=device
                )
                grad_values.index_add_(0, inverse, grad_to_buffer)
                grad_indices = grad_indices.to("cpu")
                grad_values = grad_values.to("cpu")
                time4 = time.time()
                emb_buffer.add_grad(grad_indices, grad_values)
                time5 = time.time()
                idics = idics[push_sync_mask]
                grads = grads[push_sync_mask]
                time6 = time.time()
                # grads += emb_buffer.get_grad(idics)
                time7 = time.time()
                ...
                self.time_list[5] += time6-time5
                self.time_list[6] += time7-time6
#---------------------------------------------------------------------------------------
                # will send grad to each corresponding trainer
                if self._world_size > 1:
                    # get idx split from kvstore
                    idx_split = kvstore.get_partid(emb.data_name, idics.cpu())
                    idx_split_size = []
                    idics_list = []
                    grad_list = []
                    # split idx and grad first
...
class SparseAdam(DistSparseGradOptimizer):
    ...
    def __init__(self, params, lr, betas=(0.9, 0.999), eps=1e-08,
                 local_gradient_buffer=None, local_start=None, local_end=None):
